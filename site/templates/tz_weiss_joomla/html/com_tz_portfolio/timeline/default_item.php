        ...
        <?php
            ...
            elseif($params -> get('tz_timeline_time_type') == 'month-year'):
                if(($i == 0) OR ($i != 0 AND $list[$i-1] -> tz_date  != $list[$i] -> tz_date)):
                    $strMonth       = JHtml::_('date', strtotime($row -> tz_date), JText::_('COM_TZ_PORTFOLIO_DATE_FORMAT_LC2'));
                    $strMonth       = ucfirst($strMonth);
                    $year   = $row -> year;
                    if($params -> get('tz_filter_type','tags') == 'categories'){
                        foreach($categories as $catId){
        ...
        <div id="tzelement<?php echo $row -> id;?>" class="element <?php echo $class.$tzItemClass.$tzItemFeatureClass;?>"
            ...
            <?php if( (isset($listMedia[0] -> type) AND $listMedia[0] -> type != 'quote'
                ...
                <div class="TzPortfolioDescription">
                    ...
                    <?php if(!$mediaType OR $mediaType == 'image' OR $mediaType == 'video'):?>
                    ...
                    <?php  if ($params->get('show_intro',1) AND !empty($row -> introtext) AND $mediaType == 'link') :?>
                    ...
                    <?php if (($params->get('show_author',1)) or ($params->get('show_create_date',1) && ($mediaType == 'imagegallery' OR $mediaType == 'audio')) or ($params->get('show_modify_date',1)) or ($params->get('show_publish_date',1)) or ($params->get('show_parent_category',1)) or ($params->get('show_hits',1))) : ?>
                    <div class="muted TzArticle-info">
                        <?php endif; ?>

                        <?php if($mediaType == 'imagegallery' OR $mediaType == 'audio'):?>
                            <?php if ($params->get('show_create_date',1)) : ?>
                                <div class="TzPortfolioDate">
                                    <?php echo JHtml::_('date', $row->created, JText::_('DATE_FORMAT_LC2')); ?>
                                </div>
                ...
                <?php endif; ?>
                    ...
                    <?php if($params -> get('show_readmore',1)):?>
                        ...
                        <?php endif;?>
                        <?php if($mediaType != 'imagegallery' AND $mediaType != 'audio'):?>
