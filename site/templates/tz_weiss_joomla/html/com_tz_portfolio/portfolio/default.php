...
$doc -> addScript(PLAZART_TEMPLATE_REL.'/js/resizeimage.js');
?>
...
//            if(jQuery(window).width()>979){
                var itemMargin      =  jQuery('.element .TzInner').outerWidth(true) - jQuery('.element .TzInner').outerWidth();
                contentWidth += itemMargin;
                jQuery('#tzportfolio')
                    .width(contentWidth)
                    .css('margin-left','-'+itemMargin/2+'px');
//            }
            ...
            $container.imagesLoaded(function(){
                ...
                jQuery('#portfolio .element').map(function(){
                    if(jQuery(this).find('.TzInner').hasClass('image') || jQuery(this).find('.TzInner').hasClass('video')){
                        widthStage = jQuery (this).width();
                        heightStage = jQuery (this).find('.TzInner').outerHeight();
                        tzimage = new Image();
...
<!--                <div class="filter-title TzFilter">--><?php //echo JText::_('COM_TZ_PORTFOLIO_FILTER');?><!--</div>-->
                ...
                <div id="filter" class="option-set clearfix" data-option-key="filter">
                    <a href="#show-all" data-option-value="*" class="btn btn-small selected"><?php echo JText::_('TPL_TZ_WEISS_SHOW_ALL');?></a>
            ...
            <div class="option-combo" data-toggle="tooltip" title="<?php echo JText::_('COM_TZ_PORTFOLIO_SORT')?>">
...
<!--                <div class="filter-title">--><?php //echo JText::_('COM_TZ_PORTFOLIO_LAYOUT');?><!--</div>-->
                ...
                <?php
                    if(count($params -> get('layout_type',array('masonry','fitRows','straightDown')))>0):
                        foreach($params -> get('layout_type',array('masonry','fitRows','straightDown')) as $i => $layoutType):
                ?>
                        <a class="btn btn-small<?php if($i == 0) echo ' selected';?>" href="#<?php echo $layoutType?>" data-option-value="<?php echo $layoutType?>">
                            <?php echo $layoutType?>
                        </a>
                    <?php endforeach;?>
                <?php endif;?>
