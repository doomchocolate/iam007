...
<?php if( (isset($listMedia[0] -> type) AND $listMedia[0] -> type != 'quote'
    ...
    <div class="muted TzArticleBlogInfo">
        ...
        <?php if ($params->get('show_author',1) && !empty($this->item->author )) : ?>
            <span class="TzBlogCreatedby hasTooltip" title="<?php echo JText::_('TPL_TZ_WEISS_WRITTEN_BY');?>">
        ...
        <?php if ($params->get('show_category',1)) : ?>
            <span class="TzBlogCategory hasTooltip" title="<?php echo JText::_('JCATEGORY')?>">
        ...
        <?php if ($params->get('show_parent_category') && $this->item->parent_id != 1) : ?>
            <span class="TzParentCategoryName hasTooltip" title="<?php echo JText::_('TPL_TZ_PARENT_CATEGORY')?>">
        ...
        <?php if ($params->get('show_hits')) : ?>
            <span class="TzBlogHits hasTooltip" title="<?php echo JText::_('ARTICLE_HITS')?>">
                <i class="fa fa-eye"></i>
        ...
        <span class="TzBlogCreate hasTooltip" title="<?php echo JText::_('TPL_TZ_WEISS_CREATED_DATE')?>">
          <span class="date"><i class="fa fa-clock-o"></i> <?php echo JHtml::_('date', $this->item->created, JText::_('TPL_TZ_WEISS_DATE_FORMAT_LC3')); ?></span>
        </span>
        <?php endif; ?>

        <?php if ($params->get('show_modify_date',1)) : ?>
        <span class="TzBlogModified hasTooltip" title="<?php echo JText::_('TPL_TZ_WEISS_LAST_UPDATED');?>">
        ...
        <span class="TzBlogPublished hasTooltip" title="<?php echo JText::_('TPL_TZ_WEISS_PUBLISHED_DATE');?>">
            <i class="fa fa-clock-o"></i>
    ...
    ?>
        ...
        ?>
            <a class="TzReadmore<?php if($params -> get('tz_use_lightbox') == 1) echo ' fancybox fancybox.iframe';?> hasTooltip"
               href="<?php echo $link; ?>" title="<?php echo JText::_('COM_TZ_READ_MORE');?>">
    ...
    if($params -> get('show_intro',1)){
        if($this -> item -> introtext){
            if(preg_match('/(.*?)(<\/([a-z]+|h[1-6])>)$/msi',trim($this -> item -> introtext),$match)){
                echo preg_replace('/(.*?)(<\/([a-z]+|h[1-6])>)$/msi','$1'.$titleReadmore.'$2',trim($this -> item -> introtext));
            }else{
                echo $this->item->introtext.$titleReadmore;
            }
        }
    }else{
        echo $titleReadmore;
