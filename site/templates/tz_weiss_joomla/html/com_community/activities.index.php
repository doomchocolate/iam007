...
<script type="text/javascript">
    jQuery(document).ready(function(){
        if(jQuery('#community-wrap').find('> .js-toolbar').length && jQuery('#community-wrap').find('.cMain > .joms-postbox').length){
            jQuery('#community-wrap').find('> .js-toolbar').insertBefore(jQuery('#community-wrap > .cContent .joms-postbox'));
        }
    });
</script>
...
<div class="cActivity-LoadMore" id="activity-more">
    <?php if ($showMoreActivity && !CFactory::getConfig()->get('infinitescroll',0)) { ?>
      <a class="more-activity-text btn btn-small" href="javascript:void(0);" onclick="joms.activities.more();"><?php echo JText::_('TPL_TZ_WEISS_COM_COMMUNITY_MORE'); ?></a>
    <?php } ?>
    <div class="loading"></div>
</div>
<?php if ($my->id != 0) { ?>
    <script>
        joms.activities.infinitesScrollenable = '<?php echo CFactory::getConfig()->get("infinitescroll",0)?>';
