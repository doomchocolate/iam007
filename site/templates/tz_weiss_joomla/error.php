...
if($templateParams->get('webmaster_contact_type', 'email') == 'email') {
	...
	preg_match($pattern, '<a href="mailto:'.$webmaster_email.'">'.JText::_('TPL_TZ_LANG_CONTACT_WEBMASTER').'</a>', $regs, PREG_OFFSET_CAPTURE);
	...
	$webmaster_contact  = '<a href="' . ($templateParams->get('webmaster_contact', '')) . '">' . JText::_('TPL_TZ_LANG_CONTACT_WEBMASTER') . '</a>';
}
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="<?php echo $uri->base(); ?>templates/<?php echo $this->template; ?>/fonts/OpenSansLight/stylesheet.css" />
	<link rel="stylesheet" type="text/css" href="<?php echo $uri->base(); ?>templates/<?php echo $this->template; ?>/css/themes/default/error.css" />
...
<body>
	<div id="tzPage" class="TzErrorPage">
	    <div id="tzPageTop">
	   		<?php if ($logo_type !=='none'): ?>
	   		     <?php if($logo_type == 'css') : ?>
	   		     <a href="<?php echo $this->baseurl; ?>/index.php" id="tz-logo" class="css-logo">
	   		     	<?php echo $logo_text . ' - ' . $logo_slogan; ?>
	   		     </a>
	   		     <?php elseif($logo_type == 'text') : ?>
	   		     <a href="<?php echo $this->baseurl; ?>/index.php" id="tz-logo" class="text-logo pull-left">
	   				<span><?php echo $logo_text; ?></span>
	   		        <small class="tz-logo-slogan"><?php echo $logo_slogan; ?></small>
	   		     </a>
	   		     <?php elseif($logo_type == 'image') : ?>
	   		     <a href="<?php echo $this->baseurl; ?>/index.php" id="tz-logo pull-left">
	    ...
	    <div id="tzPageWrap">
		    <div id="frame">
		        <div id="errorDescription">
		            <h2><?php echo $this->error->getCode(); ?><span class="red"><?php echo JText::_('TPL_TZ_LANG_ERROR_INFO'); ?> </span> </h2>
		            <h3><?php echo JText::_('TPL_TZ_LANG_ERROR_DESC'); ?></h3>
