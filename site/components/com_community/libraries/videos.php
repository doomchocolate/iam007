		...
		else
		{
			...
			foreach ($videos as $video)
			{
				...
				if ($videoFilename)
				{
					...
					$params->set('url', $table->getViewURI());
...
// 		$data[]	= '-metadatacreator:' . 'Jomsocial.com';
	...
	static public function getActivityTitleHTML($act)
	{
		...
		if($param->get('style') == COMMUNITY_STREAM_STYLE || empty($act->content) && !is_null($groupTitle) ){
			$html .= JText::sprintf('COM_COMMUNITY_ACTIVITY_VIDEO_SHARE_TITLE',$act->params->get('video_url'),$groupTitle);
		} else if($param->get('action', 'upload')) {
			$html .= JText::_('COM_COMMUNITY_ACTIVITY_VIDEO_UPLOAD_TITLE');
		}
