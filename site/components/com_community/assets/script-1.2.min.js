...
}catch(b){}});}},sharebox:{showFileUpload:function(a,c){var b='jax.call("community", "files,ajaxFileUploadForm","'+a+'",'+c+")";cWindowShow(b,"",600,400);
},uploadVideo:function(b,a){var c="jax.call('community', 'videos,ajaxUploadVideo', '"+b+"', '"+a+"');";cWindowShow(c,"",450,500);}},activities:{infiniteScrollLoaded:0,infinitesScrollenable:1,init:function(){joms.jQuery('li[data-streamid] [data-action="delete"]').on("click",this,function(a){a.preventDefault();
joms.activities.remove("",joms.jQuery(this).parents("[data-streamid]").data("streamid"));});joms.activities.initMap();joms.activities.loadNewStream();if(joms.activities.infiniteScrollLoaded==0&&joms.activities.infinitesScrollenable==1){joms.activities.infiniteScroll();
}},showMap:function(e,d){if(joms.jQuery("li[data-streamid="+e+"]").find(".cMapHeatzone").length==0){var b=joms.jQuery("li[data-streamid="+e+"]").find(".cStream-Actions").width();
var a='<div class="cMapFade">';a+='<div class="cMapHeatzone"> </div>';a+='<div class="cMapFiller"></div>';a+='<img src="maps.google.com/maps/api/staticmap?center='+d+"&amp;zoom=14&amp;size="+b+"x150&amp;sensor=false&amp;markers=color:red|"+d+'" />';
a+='<img src="maps.google.com/maps/api/staticmap?center='+d+"&amp;zoom=5&amp;size="+b+"x150&amp;sensor=false&amp;markers=color:red|"+d+'" />';a+='<img src="maps.google.com/maps/api/staticmap?center='+d+"&amp;zoom=2&amp;size="+b+"x150&amp;sensor=false&amp;markers=color:red|"+d+'" />';
...
var a='<img src="maps.google.com/maps/api/staticmap?center='+d+"&amp;zoom=5&amp;size="+b+"x250&amp;sensor=false&amp;markers=color:red|"+d+'" />';if(joms.jQuery(".video-map").height()==0){joms.jQuery("#video-"+e).find(".video-map-location").html(a);
joms.jQuery("#video-"+e).find(".video-map").css({height:"auto"});}else{joms.jQuery("#video-"+e).find(".video-map-location").html("");joms.jQuery("#video-"+e).find(".video-map").css({height:"auto"});
}}},getContent:function(a){jax.call("community","activities,ajaxGetContent",a);},setContent:function(a,b){joms.jQuery("#profile-newsfeed-item-content-"+a).html(b).removeClass("small profile-newsfeed-item-action").addClass("newsfeed-content-hidden").slideDown();
...
});joms.jQuery(".newsfeed-map-heatzone").live("mouseout",function(a){joms.jQuery(this).parent().find("img:eq(1)").fadeIn(0);});}}},more:function(){var f=joms.jQuery(".cMain > #activity-stream-container ul.list-unstyled");
if(f.length<=0){f=joms.jQuery("#activity-stream-container ul.list-unstyled");}var b="";if(f.find("li[data-streamid]").length>0){b=f.find("li[data-streamid]").last().data("streamid");
...
}var c=f.data("filter");var a=f.data("filterid");joms.jQuery("#activity-more .loading").show();jax.call("community","activities,ajaxGetOlderActivities",b,c,a,f.data("filter-value"));
},prependOldStream:function(a){container=joms.jQuery(".cMain > #activity-stream-container");if(container.length<=0){container=joms.jQuery("#activity-stream-container");
}joms.jQuery(a).children("li").appendTo("#activity-stream-container > ul.list-unstyled");container.find("#activity-more .more-activity-text").show();container.find("#activity-more .loading").hide();
},loadNewStream:function(){if(joms.jQuery(".joms-latest-activities-container").data("actid")>-1){joms.jQuery(".joms-latest-activities-container").css("display","none");
return;}var c=joms.jQuery("#activity-stream-container ul");jax.loadingFunction=function(){};var b=new Array();var a=0;joms.jQuery("#activity-stream-container > ul li").each(function(){if(joms.jQuery(this).data("streamid")){b[a]=joms.jQuery(this).data("streamid");
a++;}});b.sort(function(e,d){return d-e;});jax.call("community","activities,ajaxGetRecentActivitiesCount",b[0],c.data("filter"),c.data("filterid"),c.data("filter-value"));
...
return;}if(c>0){joms.jQuery(".joms-latest-activities-container").html('<div class="joms-load-latest"><a href="javascript:" onclick="joms.activities.showNewStream();">'+b+"</div>").show();
}else{joms.jQuery(".joms-latest-activities-container").hide();}if(a!=0){setTimeout("joms.activities.loadNewStream()",a);}},appendNewStream:function(d,b){joms.jQuery(".joms-latest-activities-container").html("").hide();
var a=joms.jQuery("#activity-stream-container");var c=a.find("#activity-more");c.prev("ul").prepend(d);},showNewStream:function(){var a=joms.jQuery("#activity-stream-container ul.list-unstyled");
jax.call("community","activities,ajaxGetRecentActivities",a.find("li[data-streamid]").first().data("streamid"),a.data("filter"),a.data("filterid"),a.data("filter-value"));
return false;},remove:function(c,a){var b="jax.call('community', 'activities,ajaxConfirmDeleteActivity', '"+c+"', '"+a+"');";cWindowShow(b,"",450,100);
},editLocation:function(a){var b="jax.call('community', 'activities,ajaxeditLocation', '"+a+"');";cWindowShow(b,"",450,100);},saveLocation:function(a,f){var e=joms.jQuery("#cWindow").find("#editlocation"),b=e.find("#editlocation-selector input"),h=e.find("#editlocation-map"),d=b.val(),g=h.data("lat"),c=h.data("lng");
if(!d||!g||!c){return;}jax.call("community","activities,ajaxSaveLocation",a,d,g,c);},removeLocation:function(a){var b="jax.call('community', 'activities,ajaxRemoveLocation', '"+a+"');";
cWindowShow(b,"",450,100);},infiniteScroll:function(){joms.jQuery(window).scroll(function(){if(joms.jQuery(window).scrollTop()==joms.jQuery(document).height()-joms.jQuery(window).height()){if(joms.activities.infiniteScrollDelay){return;
}joms.activities.infiniteScrollDelay=1;setTimeout(function(){joms.activities.infiniteScrollDelay=0;},1500);joms.activities.more();joms.activities.infiniteScrollLoaded=1;
}});}},apps:{windowTitle:"",toggle:function(a){joms.jQuery(a).children(".app-box-actions").slideToggle("fast");joms.jQuery(a).children(".app-box-footer").slideToggle("fast");
...
cWindowShow(d,"",470,300);}},notifications:{showWindow:function(){var a='jax.call("community", "notification,ajaxGetNotification", "")';cWindowShow(a,"",450,200);
...
if(a==0){joms.jQuery("#toolbar-item-notify").hide();setTimeout("cWindowHide()",1000);}}},showRequest:function(){var a='jax.call("community", "notification,ajaxGetRequest", "")';
cMiniWindowShow(a,"",450,100);},showInbox:function(){var a='jax.call("community", "notification,ajaxGetInbox", "")';cMiniWindowShow(a,"",450,100);},showUploadPhoto:function(a,b){var c='jax.call("community", "photos,ajaxUploadPhoto","'+a+'","'+b+'")';
...
alert(joms.photos.multiUpload.groupEmptyValidateMsg);return;}else{joms.photos.multiUpload.stopUploading();var g=joms.jQuery("#album-name").val().trim();
jax.call("community","photos,ajaxCreateAlbum",g,joms.photos.multiUpload.groupid);}}else{var f=joms.jQuery("#albumid option:selected").html();var h=joms.photos.multiUpload.uploadingSelectMsg.replace("%1$s","<strong>"+f+"</strong>");
joms.photos.multiUpload.displayNotice(h);joms.photos.multiUpload.assignUploadUrl(joms.photos.multiUpload.getSelectedAlbumId());}joms.photos.multiUpload.hideShowInput(true);
joms.jQuery("#photo-uploader").css("overflow","hidden");},UploadFile:function(f,g){joms.photos.multiUpload.log("[UploadFile]",g);joms.jQuery(".plupload_start").addClass("plupload_disabled");
},FileUploaded:function(k,g,f){try{var j=joms.jQuery.parseJSON(f.response);}catch(h){joms.photos.multiUpload.removeQueue(g.id);pluploader.pluploadQueue().stop();
joms.photos.multiUpload.displayNotice("<strong>Error Uploading.</strong>");}var l=pluploader.pluploadQueue().getFile(g.id);if(j!=undefined&&l!=undefined){if(j.error!=undefined&&j.photoId==""){joms.photos.multiUpload.removeQueue(g.id);
joms.jQuery("#upload-footer").show();joms.jQuery("#message-between").hide();joms.jQuery(".add-more").hide();cWindowResize();try{pluploader.pluploadQueue().stop();
}catch(h){}joms.photos.multiUpload.showThumbnail();joms.photos.multiUpload.displayNotice("<strong>"+j.msg+"</strong>");joms.jQuery(".plupload_file_name span").bind("click",function(m){joms.photos.multiUpload.editTitle(m);
});}else{joms.jQuery("#album-name").attr("albumid",j.albumId);l.imgSrc=j.info;l.photoId=j.photoId;joms.photos.multiUpload.refreshNeeded=true;}}},FilesAdded:function(f,g){joms.jQuery(".plupload_file_name span").unbind("click");
joms.jQuery(".plupload_button.plupload_start").show();joms.jQuery(".plupload_button.plupload_start").css("display","inline-block");},FilesRemoved:function(f,g){if(pluploader.pluploadQueue().files.length==0){joms.jQuery(".plupload_button.plupload_start").hide();
}},UploadComplete:function(f){joms.photos.multiUpload.uploadCompleteShow();setTimeout("joms.photos.multiUpload.showThumbnail(); joms.photos.multiUpload.enableEditTitle();",1000);
}},init:{Refresh:function(f){joms.photos.multiUpload.log("[Refresh]");},StateChanged:function(f){joms.photos.multiUpload.log("[StateChanged]",f.state==plupload.STARTED?"STARTED":"STOPPED");
},QueueChanged:function(f){joms.photos.multiUpload.log("[QueueChanged]");},UploadProgress:function(f,g){joms.photos.multiUpload.log("[UploadProgress]","File:",g,"Total:",f.total);
},FilesAdded:function(f,g){joms.photos.multiUpload.log("[FilesAdded]");plupload.each(g,function(h){joms.photos.multiUpload.log("  File:",h);});},FilesRemoved:function(f,g){joms.photos.multiUpload.log("[FilesRemoved]");
plupload.each(g,function(h){joms.photos.multiUpload.log("  File:",h);});},FileUploaded:function(f,g,h){joms.photos.multiUpload.log("[FileUploaded] File:",g,"Info:",h);
},ChunkUploaded:function(f,g,h){joms.photos.multiUpload.log("[ChunkUploaded] File:",g,"Info:",h);},Error:function(f,g){if(g.file){joms.photos.multiUpload.log("[error]",g,"File:",g.file);
}else{joms.photos.multiUpload.log("[error]",g);}}}});if((pluploader.pluploadQueue().runtime=="html4")||(joms.jQuery.browser.msie&&(parseInt(joms.jQuery.browser.version)<=7))||(joms.jQuery.browser.mozilla&&(joms.jQuery.browser.version.slice(0,3))==1.9)){joms.photos.multiUpload.getBrowserNotSupport();
...
}else{if(joms.jQuery("#albumid").val()){return joms.jQuery("#albumid").val();}else{return joms.jQuery("#album-name").attr("albumid");}}},showThumbnail:function(){var d=pluploader.pluploadQueue();
for(var b=0;b<d.files.length;b++){var c=document.createElement("img");c.id="plupload_"+d.files[b].id;c.setAttribute("class","plupupload_thumbnail");c.src=d.files[b].imgSrc;
joms.jQuery("#"+d.files[b].id+".plupload_done .plupload_file_name").append(c);}var a=joms.jQuery("#albumid option:selected").html();var e=joms.photos.multiUpload.uploadedCompleteMsg.replace("%1$s","<strong>"+a+"</strong>");
joms.photos.multiUpload.displayNotice(e);},assignUploadUrl:function(b){if(typeof pluploader=="undefined"){return false;}var a=joms.photos.multiUpload.defaultUploadUrl;
var c=pluploader.pluploadQueue();if(a.match(/&+/)){a=a+"&albumid="+b+"&batchcount="+c.files.length;}else{a=a+"?albumid="+b+"&batchcount="+c.files.length;
}joms.jQuery("#multi_uploader").pluploadQueue().settings.url=a;},assignNewAlbum:function(b,d){if(typeof pluploader=="undefined"){return false;}if(joms.jQuery("#albumid").length<=0){return false;
}if(d!=undefined){var a=d;var c=new Option(a,b);joms.jQuery("#albumid").append(c).val(b);joms.photos.multiUpload.assignUploadUrl(b);joms.jQuery("#album-name").val("");
}else{joms.jQuery("#albumid").val(b);var a=joms.jQuery("#albumid option:selected").html();}var e=joms.photos.multiUpload.uploadingCreateMsg.replace("%1$s","<strong>"+a+"</strong>");
joms.photos.multiUpload.displayNotice(e);joms.photos.multiUpload.showExistingAlbum();setTimeout("joms.jQuery('#multi_uploader').pluploadQueue().start()",500);
},showExistingAlbum:function(){joms.jQuery("#select-album").show();joms.jQuery("#new-album").hide();joms.jQuery("#newalbum").hide();if(pluploader.pluploadQueue().files.length>0){joms.jQuery(".plupload_start").removeClass("plupload_disabled");
}},createNewAlbum:function(){joms.jQuery("#select-album").hide();joms.jQuery("#new-album").show();joms.jQuery("#newalbum").show();},startUploading:function(){joms.jQuery("#multi_uploader").pluploadQueue().start();
...
}cWindowResize();},uploadCompleteShow:function(){joms.jQuery("div#upload-footer").show();cWindowResize();var a=pluploader.pluploadQueue();joms.ajax.call("photos,ajaxUpdateCounter",[joms.photos.multiUpload.getSelectedAlbumId(),JSON.stringify(a)]);
...
cWindowAutoResize();});},ajaxImgSelect:function(){var b=joms.jQuery("#large-avatar-pic").height();var a=160;if(b<160){a=b;}if(b>160){b=160;}joms.jQuery("#large-avatar-pic").imgAreaSelect({handles:true,aspectRatio:"1:1",x1:0,y1:0,x2:a,y2:b,show:false,hide:true,enable:false,parent:"#cWindowContentWrap",minHeight:64,minWidth:64,onInit:joms.photos.previewThumb,onSelectChange:joms.photos.previewThumb});
...
joms.jQuery(".photo_slider").css("width",f+"px");setTimeout(function(){joms.photos.photoSlider.switchPhoto();},0);},updateConfig:function(b){var a=["moveSpace","partialOpacity","fullOpacity","intervalTime"];
...
joms.gallery.displayPhoto(jsPlaylist.photos[joms.gallery.getPlaylistIndex(a)]);}},switchPhoto:function(){if(joms.photos.photoSlider.controlObj==undefined||joms.photos.photoSlider.controlObj==""){return false;
}var c=document.location.href;d=(decodeURI((RegExp("photoid=(.+?)(&|$)").exec(c)||[,null])[1]));if(d==""){var d=joms.photos.photoSlider.controlObj.find(h).eq(0).attr("id");
}joms.photos.photoSlider.controlObj.find("img").removeClass(joms.photos.photoSlider.img_thumbClass);joms.photos.photoSlider.controlObj.find('img[id="'+d+'"]').addClass(joms.photos.photoSlider.img_thumbClass);
var h=joms.jQuery("img#photoSlider_thumb"+d).css("opacity",joms.photos.photoSlider.fullOpacity);joms.jQuery(".image_thumb").not(h).css("opacity",joms.photos.photoSlider.partialOpacity);
var l=h.closest("li"),k=(l.position()||{}).left,j=l.closest("ul"),b=(j.position()||{}).left,g=j.width(),a=j.closest("div"),e=a.width(),f=e/2-l.outerWidth(true)/2;
if(g<e){b=e/2-g/2;}else{b=Math.max(0-(g-e),Math.min(0,0-k+f));}j.stop().animate({left:b});},updateThumb:function(c,a){var b='img[id="'+joms.photos.photoSlider.img_thumbId+c+'"]';
if(joms.jQuery(".slider-gallery").find(b).length==1){joms.jQuery(".slider-gallery").find(b).attr("src",a);}},removeThumb:function(c){joms.jQuery("#cPhoto"+c).remove();
var a=joms.photos.photoSlider.controlObj.find("img").length;var b=79;joms.photos.photoSlider.controlObj.css("width",(a*b)+"px");},reset:function(a){var b=joms.photos.photoSlider.parentElem.offset();
if((a.pageX>b.left&&a.pageX<(b.left+joms.photos.photoSlider.getValue(joms.photos.photoSlider.parentElem.css("width"))))&&(a.pageY>b.top&&a.pageY<(b.top+joms.photos.photoSlider.getValue(joms.photos.photoSlider.parentElem.css("height"))))){return false;
...
cWindowShow(b,"",450,100);},connect:function(a){var b='jax.call("community", "friends,ajaxConnect", '+a+")";cWindowShow(b,"",450,140);},addNow:function(){var a=jax.getFormValues("addfriend");
...
}else{if(joms.jQuery("#wall-containter .cComment:last").length>0){joms.jQuery("#wall-containter .cComment:last").after(a);}else{joms.jQuery("#wall-containter").prepend(a);
}}},remove:function(a,b,c){if(confirm("Are you sure you want to delete this wall?")){jax.call("community",a+",ajaxRemoveWall",b,c);joms.jQuery("#wall_"+b).fadeOut("normal",function(){joms.jQuery(this).remove();
});}},update:function(b,a){cWindowHide();joms.jQuery("#wall_"+b).replaceWith(a);},save:function(b,a){jax.call("community","system,ajaxUpdateWall",b,joms.jQuery("#wall-edit-"+b).val(),a);
},edit:function(b,a){if(joms.jQuery("#wall-edit-"+b).val()!=null){joms.jQuery("#wall-message-"+b).show();joms.jQuery("#wall-edit-container-"+b).children().remove();
}else{joms.jQuery("#wall-message-"+b).hide();joms.jQuery("#wall_"+b+" div.content").prepend('<span id="wall-edit-container-'+b+'"></span>').prepend('<div class="loading" style="display:block;float: left;"></div>');
...
joms.miniwall.remove(a);});joms.jQuery('a[href="#showallcomments"]').live("click",function(){var a=joms.jQuery(this).parents("li").data("streamid");jax.call("community","system,ajaxStreamShowComments",a);
return false;});joms.jQuery(".cStream-Form textarea").width(joms.jQuery(".cStream-Form").width()-12);},add:function(e,c){if(typeof c=="undefined"){return;
}var d=null;var b=null;if(joms.miniwall.type(c)=="cModule-ActivityStream"){b=joms.jQuery("#cModule-ActivityStream").find("#wall-cmt-"+e);}else{b=joms.jQuery(".cMain").find("#wall-cmt-"+e);
}var a=b.find("textarea");if(!a.data("inittextntags")){joms.miniwall._add(e,b,a.val());return;}a.textntags("val",function(f){joms.miniwall._add(e,b,f);
});},_add:function(c,b,a){a=joms.jQuery.trim(a);if(a.length>0){b.find(".wall-coc-form-action.add").attr("disabled",true);b.find(".wall-coc-errors").hide();
jax.loadingFunction=function(){b.find("textarea").attr("disabled",true);b.find(".wall-coc-form-actions").append('<em class="wall-cmt-loading">Posting...</em>');
...
};jax.call("community","system,ajaxStreamAddComment",c,a);}},insert:function(b,a){joms.jQuery("#wall-cmt-"+b+" .wallform").before(a);joms.jQuery("#wall-cmt-"+b+" .wallnone").removeClass("wallnone");
...
},cancel:function(b,a){joms.jQuery("#wall-cmt-"+b+" .wall-coc-errors").hide();joms.jQuery("#wall-cmt-"+b+" .wall-coc-form-action.add").removeAttr("disabled");
...
joms.miniwall.cancel(a);},saveEdit:function(e,d,b){var c=null;if(joms.miniwall.type(b)=="cModule-ActivityStream"){c=joms.jQuery("#cModule-ActivityStream");
}else{c=joms.jQuery(".cMain");}var a=c.find("li[data-streamid="+d+"] .cStream-Form").find("textarea");if(!a.data("inittextntags")){jax.call("community","system,ajaxeditComment",e,a.val(),d);
return;}a.textntags("val",function(f){jax.call("community","system,ajaxeditComment",e,f,d);});}},utils:{textAreaWidth:function(target){with(joms.jQuery(target)){css("width","100%");
css("width",parseInt(parent().width())-parseInt(css("borderLeftWidth"))-parseInt(css("borderRightWidth"))-parseInt(css("padding-left"))-parseInt(css("padding-right"))+11);
...
},confirmBlockUser:function(a){var b='jax.call("community", "profile,ajaxConfirmBlockUser", "'+a+'");';cWindowShow(b,"",450,100);},blockUser:function(a){var b='jax.call("community", "profile,ajaxBlockUser", "'+a+'");';
cWindowShow(b,"",450,100);},confirmUnBlockUser:function(a){var b='jax.call("community", "profile,ajaxConfirmUnBlockUser", "'+a+'");';cWindowShow(b,"",450,100);
},unBlockUser:function(a,b){b=b||null;var c='jax.call("community", "profile,ajaxUnblockUser", "'+a+'", "'+b+'");';cWindowShow(c,"",450,100);},confirmIgnoreUser:function(a){var b='jax.call("community", "profile,ajaxConfirmIgnoreUser", "'+a+'");';
cWindowShow(b,"",450,100);},ignoreUser:function(a){var b='jax.call("community", "profile,ajaxIgnoreUser", "'+a+'");';cWindowShow(b,"",450,100);},confirmUnIgnoreUser:function(a){var b='jax.call("community", "profile,ajaxConfirmUnIgnoreUser", "'+a+'");';
cWindowShow(b,"",450,100);},unIgnoreUser:function(a){var b='jax.call("community", "profile,ajaxUnIgnoreUser", "'+a+'");';cWindowShow(b,"",450,100);},removeCover:function(a){var b="jax.call('community', 'profile,ajaxRemoveCover', '"+a+"');";
...
cWindowShow(a,"",450,100);},showDesc:function(){joms.jQuery("#event-discription").show();joms.jQuery("#event-description-link").hide();},submitRSVP:function(f,h){var b=joms.jQuery(h),d=b.parent(),c=b.prev("span"),g=b.val(),a=b.attr("class"),j=b.find("option:selected").text();
if(joms.events.submitRSVP.processing){return false;}d.append('<div class="noclick" style="position:absolute;top:0;right:0;left:0;bottom:0" />');setTimeout(function(){c.html("Processing...");
c.removeAttr("class");joms.events.submitRSVP.processing=true;joms.ajax.call("events,ajaxUpdateStatus",[f,b.val()],{success:function(e){joms.events.submitRSVP.processing=false;
joms.jQuery("#community-event-members").replaceWith(e);c.attr("class",b.find("option:selected").attr("class"));c.html(j);d.children(".noclick").remove();
}});},100);},save:function(){var a="jax.call('community', 'events,ajaxShowRepeatOption');";cWindowShow(a,"",450,100);},uploadAvatar:function(c,e,b){var d=null;
if(b){d='{"call":["CEvents","getEventRepeatSaveHTML"], "library":"events", "arg":{"radio":"repeattype"}}';}var a=jax.call("community","photos,ajaxUploadAvatar",c,e,d);
cWindowShow(a,"",450,100);}},profile:{confirmRemoveAvatar:function(){var a='jax.call("community", "profile,ajaxConfirmRemoveAvatar");';cWindowShow(a,"",450,100);
},setStatusLimit:function(a){joms.jQuery(a).keyup(function(){var b=parseInt(joms.jQuery(this).attr("maxlength"));if(joms.jQuery(this).val().length>b){joms.jQuery(this).val(joms.jQuery(this).val().substr(0,joms.jQuery(this).attr("maxlength")));
...
joms.jQuery("#upload-footer").show();cWindowResize();joms.jQuery("a.add-more").click(function(){joms.jQuery("#html5_uploader").pluploadQueue().splice();
joms.jQuery(".plupload_buttons").show();joms.jQuery(".plupload_file_status").hide();joms.jQuery("div#upload-footer").hide();cWindowResize();});}}});joms.jQuery(".plupload_header").remove();
joms.jQuery("span.plupload_upload_status").remove();joms.jQuery("input:file").parent().css("top","245px");joms.jQuery("#cwin_close_btn").click(function(){if(a){window.location.reload();
}});},viewFile:function(a,c){var b='jax.call("community", "files,ajaxviewFiles","'+a+'","'+c+'")';cWindowShow(b,"",600,400);},ajaxdownloadFile:function(a,c){var b='jax.call("community","files,ajaxFileDownload","'+a+'","'+c+'")';
cWindowShow(b,"",600,200);},getFileList:function(e,c,b,a,d){joms.jQuery("#load-more-btn").html("");jax.call("community","files,ajaxgetFileList",e,c,b,a,d);
},updateFileList:function(a,b){currentFiles=joms.jQuery("#"+b).html();newFiles=currentFiles+a;joms.jQuery("#"+b).html(newFiles);},viewMore:function(a,c){var b='jax.call("community","files,ajaxviewMore","'+a+'","'+c+'")';
cWindowShow(b,"",600,200);},loadFile:function(d,c,b,a,e){joms.jQuery("#load-more-btn").html("");jax.call("community","files,ajaxgetFileList",d,c,b,a,e);
},searchFileUpdate:function(a,b){joms.jQuery(joms.jQuery("ul.cTabNav li.active").children().attr("href")).html(a);},startdownload:function(a){joms.jQuery("<iframe>",{src:a,id:"iframe"}).appendTo("body");
var c=document.getElementById("iframe");c=c.contentWindow||c.contentDocument;if(c.document){c=c.document;}var b=setInterval(function(){if(c.readyState=="complete"){clearInterval(b);
cWindowHide();joms.jQuery("#iframe").remove();}},1000);}},cover:{type:null,parentId:null,position:0,init:function(){joms.jQuery(".js-navbar-options, .js-navbar-icon").dropdown({toggle:false});
var b=joms.jQuery(".js-bar-collapse");b.collapse({toggle:false});joms.jQuery(".js-bar-collapse-btn").on("click",function(d){d.preventDefault();b.collapse("toggle");
});var a=joms.jQuery(".js-collapse");a.collapse({toggle:false});joms.jQuery(".js-collapse-btn").on("click",function(d){d.preventDefault();a.collapse("toggle");
});var c=joms.jQuery(".js-collapse-about");c.collapse({toggle:false});joms.jQuery(".js-collapse-about-btn").on("click",function(d){d.preventDefault();c.collapse("toggle");
});},select:function(b,d,a){joms.cover.type=joms.jQuery(b).data("cover-context");joms.cover.parentId=d;var c='jax.call("community","photos,ajaxChangeCover","'+joms.cover.type+'","'+joms.cover.parentId+'")';
cWindowShow(c,"",600,400);},openalbum:function(a,b){jax.call("community","photos,ajaxGetPhotoList",a,b);},showlist:function(a){joms.jQuery("#coverphotolist").html(a);
joms.jQuery("#coverphotolist").show();joms.jQuery("#coveralbumlist").hide(function(){});},setCover:function(a){jax.call("community","photos,ajaxSetPhotoCover",joms.cover.type,a.id,joms.cover.parentId);
...
var c='<button class="btn btn-primary" onClick="joms.cover.repositionCancel('+h+",'"+a+"')\"><span>"+d+'</span></button><button class="btn btn-primary ml-6" onClick="joms.cover.saveReposition()"><span>'+b+"</span></button>";
...
},saveReposition:function(){jax.call("community","photos,ajaxSetPhotoPhosition",joms.cover.type,joms.cover.parentId,joms.cover.position);joms.jQuery(".js-focus-gradient").show();
joms.jQuery(".js-focus-change-cover").css({display:""});joms.jQuery(".span4.text-right").html("");joms.jQuery("img#"+joms.cover.parentId).draggable("destroy");
joms.jQuery(".js-focus-change-cover a.btn.hidden").removeClass("hidden");}},share:{popup:function(a){joms.jQuery(document).off("click","#cWindowAction button.btn-primary");
var b="jax.call('community', 'activities,ajaxSharePopup','"+a+"');";cWindowShow(b,"",450,300);},add:function(a,b){joms.jQuery(document).off("click","#cWindowAction button.btn-primary");
jax.call("community","activities,ajaxAddShare",a,b);},map:function(a){var b="jax.call('community', 'activities,ajaxShowMap','"+a+"');";cWindowShow(b,"",650,300);
...
joms.tooltip.setup();joms.apps.initToggle();joms.plugins.initialize();if(joms.jQuery(".list-unstyled li").length){joms.miniwall.initialize();}var a=window.location;
